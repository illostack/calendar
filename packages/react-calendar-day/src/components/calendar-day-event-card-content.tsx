...
const calendarEventCardVariants = cva(
  "h-full w-full relative pointer-events-none shadow-md flex-col pl-4 pr-2 rounded-[var(--calendar-radius)] py-1.5 text-xs before:absolute before:left-1.5 before:inset-y-1.5 before:w-1 before:rounded-lg ring-1 ring-foreground/5 flex overflow-hidden group-data-[event-state=active]:ring-2 group-data-[event-state=active]:ring-primary group-data-[event-state=active]:shadow-xl group-data-[event-state=active]:ring-offset-transparent select-none",
  {
    variants: {
      color: {
        green:
